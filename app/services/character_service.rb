class CharacterService
  ...
  def validate_level
    ...
    level = level.to_i
    return unless level < 1 || level > 20
  ...
  def validate_archetype
    ...
    return if archetype_name.blank?

    return if Archetypes.names.include?(archetype_name.to_sym)

    @errors << "Invalid archetype: #{archetype_name}"
  end

  def validate_proficiencies
    return if @params[:proficiencies].blank?
  ...
  def validate_ability_scores
    ...
    abilities.each do |ability|
      ...
      value = value.to_i
      @errors << "#{ability.capitalize} must be between 1 and 30" if value < 1 || value > 30
    end
  end
  ...
  def validate_current_spell_list
    ...
    begin
      ...
      @errors << "Spell list does not belong to this character" unless spell_list.character_id == @character.id
  ...
  def apply_saving_throw_proficiencies(archetype_obj)
    ...
    saving_throws.each do |saving_throw|
      updated_proficiencies << saving_throw unless updated_proficiencies.include?(saving_throw)
    end
  ...
  def ensure_spellcasting_consistency(archetype_obj)
    ...
    archetype_obj.spell_slots_at_level(level)
  ...
  def archetype
    archetype_name = @params[:archetype] || @character.archetype&.name
    return nil if archetype_name.blank?
  ...
  def extract_valid_skills(archetype_obj)
    ...
    return DND_SKILLS if skill_prof_text =~ /^any\s+\d+\s+skills?$/i

    # Handle case where it says "Any three skills" - all skills are valid
    return DND_SKILLS if skill_prof_text.downcase.include?("any") && skill_prof_text.downcase.include?("skill")
  ...
  def level_changed?
    return false if @params[:level].blank?
