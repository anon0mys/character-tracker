class SpellListItem < ApplicationRecord
  ...
  def character_can_learn_spell
    return if spell.archetypes.include?(spell_list.character.archetype.name.to_s)
  ...
  def spell_list_not_exceeding_prepared_limit
    ...
    existing_items = existing_items.where.not(id:) if persisted?
    ...
    return unless new_spell_count > max_prepared
