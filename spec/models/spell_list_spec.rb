...
RSpec.describe SpellList do
  ...
  context "relationships" do
    ...
    it { is_expected.to have_many(:spells).through(:spell_list_items) }
  end
  ...
  describe "#add_spell" do
    ...
    it "adds a spell that includes the character archetype" do
      spell_list.add_spell(valid_spell)
      expect(spell_list.spells.first).to eq(valid_spell)
    end

    it "does not add the spell if it is already in the list" do
      spell_list.add_spell(valid_spell)
      spell_list.add_spell(valid_spell)
      expect(spell_list.spells.count).to eq 1
    end

    it "raises an error if the spell is not available to an archetype" do
    ...
    context "spell list prepared limit" do
      ...
      it "prevents adding spells beyond the prepared limit" do
        max_prepared = cleric.max_spells_prepared
      ...
      it "does not count cantrips against the prepared limit" do
        max_prepared = cleric.max_spells_prepared
